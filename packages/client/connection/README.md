...
A deployment that serves the surface to arbitrary clients binds one HTTP Basic credential to the same gate: set `basicAuthFile` in the Connection row to a file holding a single `user:password` line (a mode-600 file outside the repository), and every request — the root index, `/api`, and WebSocket handshakes — accepts the matching `Authorization: Basic` header. The first successful root exchange writes the same authority-bound signed cookie the token flow writes, so the browser moves to the cookie session at once, and each 401 carries `WWW-Authenticate: Basic` so any browser can prompt for the credential. Without a bound credential the gate stays on the launch-token and cookie flow only; the one-time URL remains a local recovery path in either mode. A missing or malformed credential file fails Connection load.
...
Before authentication, every request still passes `src/api-request-trust.ts`. Its `Host` must be loopback or match a `trustedHosts` entry: exact on `host:port`, any port on port-less entries, both sides WHATWG-normalized. The same list is injected into every served page as the `__DSH_TRUSTED_HOSTS__` global, so the browser's own page classification — the `connection` handle's privileged-surface flag — accepts a page served from a declared authority, the very pages whose requests this fence admits, instead of loopback hostnames only. An attached `Origin` must equal that Host and `sec-fetch-site: cross-site` is refused. Malformed configured authorities fail plugin load. These checks defend DNS rebinding and cross-site browser requests; they never establish identity. A failed Host/Origin check returns 403, while a trusted but unauthenticated request returns 401. `dsh web --host 0.0.0.0` remains unsupported. Decision records: [browser request trust](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.md) and [browser token authentication](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.md).
