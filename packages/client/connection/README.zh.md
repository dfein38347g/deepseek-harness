...
把该 surface 服务给任意 client 的部署，可以给同一门禁绑定一个 HTTP Basic 凭据：在 Connection 行上把 `basicAuthFile` 设为一个只含一行 `user:password` 的（仓库之外的）mode-600 文件，之后每个请求——根 index、`/api` 与 WebSocket 握手——都接受匹配的 `Authorization: Basic` header。第一次成功的根路径交换写入令牌流写入的同一 authority 绑定签名 cookie，浏览器随即转入 cookie 会话；每个 401 都携带 `WWW-Authenticate: Basic`，任何浏览器都可以弹出凭据输入。未绑定凭据时，门禁只走启动令牌与 cookie 流；一次性 URL 在两种模式下都保留为本地恢复路径。凭据文件缺失或畸形会让 Connection 加载失败。
...
认证之前，每个请求仍经过 `src/api-request-trust.ts`。其 `Host` 必须是 loopback，或与 `trustedHosts` 条目匹配：带端口的 `host:port` 精确匹配，不带端口的条目匹配任意端口，两侧均经 WHATWG 归一化。同一列表作为 `__DSH_TRUSTED_HOSTS__` 全局变量注入每个已服务页面，使浏览器自身的页面判定——`connection` 句柄的特权面标志——接受来自已声明 authority 提供的页面，恰是本网关放行的那些页面，而不再只认 loopback hostname。若附带 `Origin`，它必须等于该 Host；`sec-fetch-site: cross-site` 一律拒绝。畸形配置 authority 会让插件加载失败。这些检查防御 DNS rebinding 与跨站浏览器请求，绝不建立身份。Host/Origin 校验失败返回 403；Host 可信但未认证的请求返回 401。`dsh web --host 0.0.0.0` 仍不受支持。决策记录：[浏览器请求信任](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.zh.md)与[浏览器令牌认证](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.zh.md)。
