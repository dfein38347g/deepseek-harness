...
async function setup(
  ...
  await ctx.plugin(SandboxPolicyService, { mode: 'workspace-write', workspaceRoot: spillDir, network: 'inherit' })
...
describe.skipIf(!pwshAvailable())('SandboxPwshExecutor', () => {
  ...
  const RO: SandboxExecutionPolicy = { mode: 'read-only', workspaceRoot: '/ws', network: 'inherit' }
  ...
  it('danger-full-access bypasses confine entirely and stamps full-access facts', async () => {
    const { executor, calls } = await setup()
    const result = await executor.run(executor.resolve({ command: 'echo full', sandboxPolicy: { mode: 'danger-full-access', workspaceRoot: '/ws', network: 'inherit' } }))
    expect(result.exitCode).toBe(0)
  ...
  it('danger-full-access background runs bypass confine and carry no facts', async () => {
    ...
    const proc = executor.start(executor.resolve({
      ...
      sandboxPolicy: { mode: 'danger-full-access', workspaceRoot: '/ws', network: 'inherit' },
    }))
    await proc.done
    expect(calls).toHaveLength(0)
