...
export class FakeApiClient {
  ...
  onRemove: (payload: unknown) => Promise<RemoteResult<{ removed: true }>> = () => Promise.resolve(ok({ removed: true }))
  ...
  sessionRemotes(): RuntimeRemotes {
    return {
      ...
      session: {
        ...
        remove: payload => this.record('session.remove', payload, this.onRemove(payload)),
