...
`dsh-tool-skill` 在存活会话中第一个观察到非空完整视图的 `agent/pre-step` 注入初始的持久 user-role `<system-reminder>`。目录只包含已排序的 skill `name` 和规范化、经 XML 转义的 `description`；不包含正文、路径、来源、提供方或路由提示。发现通过 `SkillLookupOptions` 转发该步骤的 abort signal。`catalogDescriptionMaxLength` 是消费方用于 description 上限的配置，默认值为 `500`，整数最小值为 `3`。`publishCatalog`（默认 `true`）在步骤边界整体关闭目录：`false` 时任何步骤都不发布目录，此前发布的目录从每个步骤窗口退役，而 `skill` 加载器与 `/<name>` 手势保持可用。`alwaysInclude`（默认为空）固定一组精选 skill：`publishCatalog` 为 `false` 时，目录在每个步骤边界只发布那些可见且模型可调用的固定名称，即使完整列表被抑制也始终被广告；当该集合不再解析出任何 skill 时目录退役，`publishCatalog` 为 `true` 时该键无效。
